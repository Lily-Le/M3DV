...
import numpy as np
from mylibrary.dataloader.path_manager import PATH
from mylibrary.utils.misc import rotation, reflection, crop, random_center, _triple

INFO = PATH.info

TEST_INFO=PATH.test_info
...
class ClfDataset(Sequence):
    def __init__(self, crop_size=32, move=3, subset=[0, 1,2,3],
        ...
        index = []
        for sset in subset:
            index += list(TEST_INFO[TEST_INFO['subset'] == sset].index)
        self.index = tuple(sorted(index))  # the index in the info
        self.label = np.array([[label == s for label in LABEL] for s in TEST_INFO.loc[self.index, 'label']])
        self.transform = Transform(crop_size, move)
        self.define_label = define_label

    def __getitem__(self, item):
        name = TEST_INFO.loc[self.index[item], 'name']
        with np.load(os.path.join(PATH.test_nodule_path, '%s.npz' % name)) as npz:
