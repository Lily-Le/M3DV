...
class ClfDataset(Sequence):
    def __init__(self, crop_size=32, move=3, subset=[0, 1,2,3]
        ...
        self.label = np.array([[label == s for label in LABEL] for s in INFO.loc[self.index, 'label']])
        #self.label=INFO.loc[self.index,'label']
...
class ClfSegDataset(ClfDataset):
    ...
    def __getitem__(self, item):
        ...
        label = INFO.loc[self.index[item], 'label'] ####label = self.label[item]
